    ...
    if port in database_signatures:
        ...
        if port == 6379:  # Redis
            ...
            if not es_info:
                try:
                    import urllib.request
                    response = urllib.request.urlopen(f"http://{host}:{port}", timeout=timeout)
...
def comprehensive_service_detection(host, port, timeout=3, enhanced=None):
    ...
    if port in [80, 443, 8080, 8443, 3000, 4000, 5000, 8000, 9000]:
        ...
        if http_info:
            service_info['service'] = http_info.get('framework') or f"HTTP ({http_info['protocol'].upper()})"
...
def scan_all_configured_ports(host, timeout=3, show_closed=False, detect_services=True):
    ...
    with ThreadPoolExecutor(max_workers=50) as executor:
        futures = {executor.submit(check_port, host, port, timeout): (port, service) 
    ...
    if detect_services and open_ports:
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:  # Fewer threads for service detection
            futures = {executor.submit(comprehensive_service_detection, host, port, timeout): port 
    ...
    print("=" * 90)
    print(f"📊 Summary: {len(open_ports)} open, {len(closed_ports)} closed out of {len(all_ports)} total")
    ...
    for list_name, details in config["port_lists"].items():
        ...
        if category_open:
            print(f"  {list_name:12}: {len(category_open)}/{category_total} open - {', '.join(map(str, sorted(category_open)))}")
        else:
...
def main():
    ...
    parser.add_argument("--show-lists", action="store_true",
                       help="Show available port lists and exit")
    ...
    try:
        if args.port:
            ...
            if is_open and detect_services:
                ...
                service_info = comprehensive_service_detection(args.host, args.port, args.timeout)
            ...
            scan_all_configured_ports(args.host, args.timeout, args.show_closed, detect_services)
