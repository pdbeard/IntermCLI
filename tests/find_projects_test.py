...
from tools.find_projects.find_projects import ConfigManager, ProjectDetector, SecurityValidator, ProjectScanner, RateLimiter, SearchEngine, ProjectGrouper, ProjectOpener, Project
...
def test_rate_limiter():
    limiter = RateLimiter(max_ops_per_second=2)
...
def test_search_engine_fuzzy():
    projects = [Project(name="Alpha", path="/a", relative_path="a", project_type="Python", last_modified=0),
                Project(name="Beta", path="/b", relative_path="b", project_type="Node.js", last_modified=0)]
    results = SearchEngine.fuzzy_search("Al", projects, key_func=lambda p: p.name)
    assert any(p.name == "Alpha" for p in results)

def test_project_grouper():
    projects = [Project(name="Alpha", path="/a", relative_path="a", project_type="Python", last_modified=1),
                Project(name="Beta", path="/b", relative_path="b", project_type="Node.js", last_modified=2)]
    grouped = ProjectGrouper.group_projects_by_type(projects)
...
def test_project_opener_handles_missing_editor(tmp_path):
    ...
    result = ProjectOpener.open_project(str(project_dir), "proj", editor="nonexistent_editor")
